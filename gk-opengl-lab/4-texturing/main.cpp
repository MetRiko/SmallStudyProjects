...
struct Egg : public EventObject, RenderObject {
	...
	bool flagRenderNormalVectors = false;
	...
	Egg& SetColor(float r, float g, float b) {
		rgb[0] = r;
		rgb[1] = g;
		rgb[2] = b;
		return *this;
	...
	Egg& SetRenderNormalVectors(bool flag) {
		flagRenderNormalVectors = flag;
		return *this;
	}
	...
	void putTexCoord(int x, int y) {
		glTexCoord2f(texTab[x][y][0], texTab[x][y][1]);
		//glTexCoord2f((float)x / N, (float)y / N);
	}
	...
	void Render() {
		...
		if (model == 1) {
			...
			for (int i = 0; i < N; ++i) {
				for (int j = 0; j < N - 1; ++j) {
					int x = i, y = j;
					putColorPoint(x, y); //putTexCoord(x, y);
					y = (j + 1) % N;
					putColorPoint(x, y); //putTexCoord(x, y);
					x = (i + 1) % N;
					putColorPoint(x, y); //putTexCoord(x, y);
			...
			if (flagRenderNormalVectors == true) {
				glEnd();
				glBegin(GL_LINES);
				glColor3f(0.0, 0.0, 0.0);
				for (int i = 0; i < N; ++i) {
...
struct LightManager {
	...
	LightManager& SetAmbient(float r, float g, float b) {
		mat_ambient[0] = r;
		mat_ambient[1] = g;
		mat_ambient[2] = b;
		glMaterialfv(GL_FRONT, GL_AMBIENT, mat_ambient);
...
struct World {
	...
	void Init() {
		...
		//lm.SetAmbient(1.0, 1.0, 1.0);

		lm.CreateLight()->SetPosition(-20.0, 0.0, 10.0).SetDiffuse(0.5, 0.0, 0.0).SetSpecular(1.0, 0.2, 0.1).SetCLQ(0.001, 0.02, 0.0001);
		lm.CreateLight()->SetPosition(20.0, 0.0, 10.0).SetDiffuse(0.0, 0.0, 0.5).SetSpecular(0.2, 0.2, 1.0).SetCLQ(0.001, 0.02, 0.0001);

		//lm.CreateLight()->SetPosition(-20.0, 0.0, 10.0).SetDiffuse(0.5, 0.0, 0.0).SetSpecular(1.0, 0.2, 0.1).SetCLQ(0.00001, 0.0005, 0.00001);
		//lm.CreateLight()->SetPosition(20.0, 0.0, 10.0).SetDiffuse(0.0, 0.0, 0.5).SetSpecular(0.2, 0.2, 1.0).SetCLQ(0.00001, 0.0005, 0.00001);
		
		//auto *obj = AddObject(new Egg<50>);
		//obj->SetRenderNormalVectors(false);

		AddObject<Axes>();
		AddObject<Egg<50>>()->SetRenderNormalVectors(false).SetColor(1.0, 1.0, 1.0);
		//AddObject<Triangle>()->SetColor(0.f, 1.f, 1.f);
	}
