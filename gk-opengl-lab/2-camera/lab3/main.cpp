...
void cameraPositionUpdate() {
	...
	//}
	//top = 1.0;
	//if (angleY > 3.14159*0.5 || angleY < -3.14159*0.5)
	//	top = -1.0;

	viewer[0] = zoom * std::cosf(aX) * std::cosf(aY);
	viewer[1] = zoom * std::sinf(aY);
	viewer[2] = zoom * std::sinf(aX) * std::cosf(aY);
}
...
void eventUpdate() {
	...
	if (status == 1) {
		angleX += delta_x * pix2angle * dpi;
		angleY += delta_y * pix2angle * dpi;
